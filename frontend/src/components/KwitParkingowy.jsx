...
const KwitParkingowy = ({ isSubmitted, onFormSubmit }) => {
  ...
  const handleSubmit = (e) => {
      ...
      const updatedData = {
          ...data,
          okres_korzystania_od: okresKorzystaniaOd?.toLocaleDateString('pl-PL', options),
          okres_korzystania_do: okresKorzystaniaDo?.toLocaleDateString('pl-PL', options),
      };
      ...
      fetch('http://localhost:4000/student/egzamin/kwitParkingowy', {
          ...
          .catch((error) => {
              ...
              toast({
                  title: 'Błąd',
                  description: `Nie udało się przesłać odpowiedzi: ${error.message}`,
              });
          });
  };
        ...
        <tr >
            <td className={tdClass} colSpan={4}>
              ...
              <div className='w-full my-2 p-1'>
                <Input className="h-full" rows={2} name="imie_i_nazwisko_goscia" disabled={isSubmitted} onChange={handleChange}/>
              </div>
            </td>
        </tr>
        <tr >
        ...
        <tr >
            <td className={tdClass} colSpan={2}>
              <div className='flex justify-start gap-3 flex-col'>
                ...
                <Input type="number" onChange={handleChange} name="nr_pokoju" className="h-full" rows={1} disabled={isSubmitted}/>
              </div>
            </td>
            <td className={tdClass} colSpan={2}>
              <h2>Samochód marki:</h2>
              <div className='w-full my-2 p-1'>
                <Input className="h-full" onChange={handleChange} name="samochod_marki" rows={2} disabled={isSubmitted}/>
              </div>
              <h2>Nr rejestracyjny:</h2>
              <div className='w-full my-2 p-1'>
                <Input className="h-full" onChange={handleChange} name="nr_rejestracyjny" rows={2} disabled={isSubmitted}/>
              </div>
            </td>
        </tr>
        <tr>
            <th className={thClass} colSpan={2}>Podpis pracownika parkingu</th>
            <td className={tdClass + "h-full"} colSpan={2}><Input type="text" onChange={handleChange} name="podpis_pracownika" required disabled={isSubmitted}/></td>
        </tr>
    </table>
